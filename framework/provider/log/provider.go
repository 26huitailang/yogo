...
// Register 注册一个服务实例
func (l *YogoLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewYogoConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewYogoSingleLog
	case "rotate":
		return services.NewYogoRotateLog
	case "console":
		return services.NewYogoConsoleLog
	case "custom":
		return services.NewYogoCustomLog
	default:
		return services.NewYogoConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *YogoLogServiceProvider) Boot(c framework.Container) error {
	return nil
}
