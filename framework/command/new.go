...
var newCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		}
		{
			...
			prompt := &survey.Input{
				Message: fmt.Sprintf("请输入版本名称(参考 https://github.com/%s/%s/releases，默认为最新版本)：", OWNER, REPO),
			}
			...
			if version != "" {
				// 确认版本是否正确
				release, _, err = client.Repositories.GetReleaseByTag(context.Background(), OWNER, OWNER, version)
				if err != nil || release == nil {
					fmt.Printf("版本不存在，创建应用失败，请参考 https://github.com/%s/%s/releases\n", OWNER, REPO)
					return nil
				}
			}
			if version == "" {
		...
		fmt.Println("yogo框架版本：", release.GetTagName())

		templateFolder := filepath.Join(currentPath, "template-yogo-"+version+"-"+cast.ToString(time.Now().Unix()))
		...
		// 获取folder下的26huitailang-yogo-xxx相关解压目录
		...
		for _, fInfo := range fInfos {
			// 找到解压后的文件夹
			if fInfo.IsDir() && strings.Contains(fInfo.Name(), "26huitailang-yogo-") {
		...
		filepath.Walk(folder, func(path string, info os.FileInfo, err error) error {
			...
			if path == filepath.Join(folder, "go.mod") {
				...
				c = bytes.ReplaceAll(c, []byte(fmt.Sprintf("module github.com/%s/%s", OWNER, REPO)), []byte("module "+mod))
				c = bytes.ReplaceAll(c, []byte("require ("), []byte(fmt.Sprintf("require (\n\tgithub.com/%s/%s ", OWNER, REPO)+version))
			...
			isContain := bytes.Contains(c, []byte("github.com/26huitailang/yogo/app"))
			if isContain {
				fmt.Println("更新文件:" + path)
				c = bytes.ReplaceAll(c, []byte("github.com/26huitailang/yogo/app"), []byte(mod+"/app"))
				err = ioutil.WriteFile(path, c, 0644)
				if err != nil {
					return err
				}
