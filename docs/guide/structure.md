...
yogo 框架不仅仅是一个类库，也是一个定义了开发模式和目录结构的框架。 yogo 希望所有使用这个框架的开发人员遵照统一的项目结构进行开发。

## 默认目录结构
默认创建的项目结构为：

```
[~/Documents/workspace/yogo_workspace/demo5]$ tree
.
├── README.md
├── app // 服务端应用地址
│   ├── console  // 存放自定义命令
...
第一步：创建当前业务的 provider。可以使用命令行 `./yogo provider new` 来创建。
```
...
create provider success, folder path: /Users/xxx/Documents/workspace/yogo_workspace/demo5/app/provider/car
